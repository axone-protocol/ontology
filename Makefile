...
# - Check
FLG_CHECK_OK := $(DST_MAKE)/check.checked
...
RDF_VALIDATE = \
  riot --validate $1
CLI = \
  PYTHONPATH=$(SRC_SCRIPT) python3 -m cli.cli $1 $2 $3 $4 $5 $6 $7 $8 $9
MARKDOWN_LINT = \
  markdownlint $1 $2 $3 $4 $5 $6 $7 $8 $9
...
## Clean:
.PHONY: clean
clean: clean-build clean-ontologies ## Clean all generated files
...
build-ontology-jsonld: check $(DST) $(OBJ_SCHEMAS_JSONLD)  ## Build the ontology in JSON-LD format
...
$(OBJ_ONTS_NT): $(DST_ONT)/%.nt: $(DST_ONT)/%.ttl
	@echo "${COLOR_CYAN}🔨 building${COLOR_RESET} ontology ${COLOR_GREEN}$@${COLOR_RESET}"
	@mkdir -p -m $(PERMISSION_MODE) $(@D)
	@${call RDF_WRITE,ntriple,$<,$@}
	@${call NT_UNIQUIFY,$@}

$(OBJ_ONTS_RDFXML): $(DST_ONT)/%.rdf.xml: $(DST_ONT)/%.ttl
	@echo "${COLOR_CYAN}🔨 building${COLOR_RESET} ontology ${COLOR_GREEN}$@${COLOR_RESET}"
	@mkdir -p -m $(PERMISSION_MODE) $(@D)
	@${call RDF_WRITE,rdfxml,$<,$@}
...
.PHONY: format-ttl
format-ttl: check $(FLG_FMT_TTLS) ## Format all Turtle files
...
.PHONY: lint-ttl
lint-ttl: check $(FLG_LINT_TTLS) ## Lint all Turtle files
...
.PHONY: lint-jsonld
lint-jsonld: check build-ontology-jsonld $(FLG_LINT_JSONLDS) ## Lint all JSON-LD files
...
$(FLG_CHECK_OK):
	...
	@for cmd in awk cool-rdf md5sum python3 riot shacl timeout; do \
		path=$$(command -v $$cmd); \
		if [ -z "$$path" ]; then \
			echo "${COLOR_CYAN}❌ ${COLOR_GREEN}$$cmd${COLOR_RESET} command is not available, please run \`nix develop\`." && exit 1; \
		else \
...
vars: ## Show relevant variables used in this Makefile
	$(foreach var,$(sort $(filter VERSION_% SRC_% DST_% OBJ_% FLG_% BIN_%,$(.VARIABLES))),$(info ${COLOR_GREEN}$(var)${COLOR_WHITE}=${COLOR_CYAN}$($(var))${COLOR_RESET}))
...
help: ## Show this help.
	...
	@echo ''
	@echo 'This Makefile runs in the ${COLOR_CYAN}Nix${COLOR_RESET} development environment.'
	@echo 'Run ${COLOR_YELLOW}direnv allow${COLOR_RESET} once, or invoke ${COLOR_YELLOW}nix develop${COLOR_RESET}.'
